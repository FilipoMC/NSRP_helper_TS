...
import { MBlacklist } from "../../../schemas/blacklist";
...
export default async function({ client, interaction }: SlashCommandProps) {
   ...
   const tempBlacklistTest = await MBlacklist.findOne({user_id: robloxUser.id});
   ...
   const blacklistDoc = await MBlacklist.create({
