...
import { IBlacklist, MBlacklist } from "../../../../../schemas/blacklist";
import revoke from "..";
...
export default async function({ client, interaction }: SlashCommandProps) {
   ...
   const blacklistDoc = await MBlacklist.findOne({user_id: robloxID});
   ...
   const updatedBlacklistDoc = await MBlacklist.findOne({user_id: robloxID});
