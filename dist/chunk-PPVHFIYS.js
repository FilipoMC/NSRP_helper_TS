...
${(await interaction.guild?.members.fetch(document.createdBy))?.user?.username}` },
  ...
  inactiveButton.onClick(
    async (subInteraction) => {
      ...
      const membersOnLoa = (await interaction.guild?.roles.fetch(main.loaRole))?.members ?? new Collection();
      const reactedMembers = (await interaction.guild?.roles.fetch(main.activityTestRole))?.members ?? new Collection();
      const LSTeam = (await interaction.guild.roles.fetch(ranks.categories.ls))?.members ?? new Collection();
      const employees = (await interaction.guild?.roles.fetch(ranks.categories.staff))?.members ?? new Collection();
      const inactiveMembers = new Collection();
      for (const [id2, member] of interaction.guild.members.cache) {
        if (!member.user.bot && employees.has(id2) && !membersOnLoa.has(id2) && !reactedMembers.has(id2) && !LSTeam.has(id2)) {
