...
**Host:** ${interaction.user}`
  ...
  for (const message2 of channelMessages) {
    if (status === "ssu") {
      if (!message2[1].pinned) {
        await message2[1].delete();
      }
    } else if (status === "ssd") {
      await message2[1].delete();
    }
  ...
  if (status === "ssu") {
    ...
    for (const message2 of channel.messages.cache) {
      if (message2[1].embeds.at(0)?.title === EMBED_TITLE) {
        const attendButton = message2[1].components.at(0)?.components.at(0);
        ...
        await message2[1].edit({ components: [editedRow] });
      }
    }
  }
